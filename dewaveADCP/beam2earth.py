...
#### 4-beam Janus ####
######################
def janus2xyz(b1, b2, b3, b4, theta, r=None, binmaptype='linear', ptch=None, roll=None, use3beamsol=True, verbose=True):
    """
    USAGE
    -----
    vx, vy, vz = janus2xyz(b1, b2, b3, b4, theta, r=None, binmaptype='linear', ptch=None, roll=None, use3beamsol=True, verbose=True)
    """
    Nz, Nt = b1.shape
...
def janus2earth(head, ptch, roll, theta, b1, b2, b3, b4, r=None, gimbaled=True, binmaptype='linear', use3beamsol=True, verbose=True):
    """
     USAGE
     -----
     [u, v, w] = janus2earth(head, ptch, roll, theta, b1, b2, b3, b4, r=None, gimbaled=True, binmaptype='linear', use3beamsol=True, verbose=True)
    ...
    if gimbaled: # Correct heading (D&S 2007, eq. A2).
        ...
        Sph2Sph3 = Sph2*Sph3
        head = head + np.asin( Sph2Sph3/np.sqrt(Cph2**2 + Sph2Sph3**2) )
        Sph1 = np.sin(head)
        Cph1 = np.cos(head)
    else:                      # Correct pitch (D&S 2007, eq. A1 Lohrmann et al. 1990, eq. A1).
        print('Fixed instrument case.')
        ptch = np.asin( (Sph2*Cph3)/np.sqrt(1 - (Sph2*Sph3)**2) )
        Sph2 = np.sin(ptch)
        Cph2 = np.cos(ptch)
...
#### 5-beam Janus ####
######################
def janus2xyz5(b1, b2, b3, b4, b5, theta, binmaptype='linear', use3beamsol=True, verbose=True):
    """
    USAGE
    -----
    vx, vy, vz = janus2xyz5(b1, b2, b3, b4, b5, theta, binmaptype='linear', use3beamsol=True, verbose=True)
    """
    Nz, Nt = b1.shape
    if binmaptype is not None:
        assert r is not None, "Must provide r if using bin-mapping."
