...
def bvarAF(b, sep=6, Lw=8, max_badfrac=0.5, mindet=1e-4, verbose=False):
    ...
    USAGE
    -----
    bvar_dewaved = bvarAF(bvel, sep=6, Lw=8, mindet=1e-4, max_badfrac=0.5, verbose=False)

    Returns de-waved, time-averaged beam variances.
    """
    nz, nt = b.shape
...
def bvelAF(b, sep=6, Lw=8, max_badfrac=0.5, mindet=1e-4, verbose=False):
    """
    USAGE
    -----
    bvvel_dewaved = bvelAF(bvel, sep=6, Lw=8, mindet=1e-4, max_badfrac=0.5, verbose=False)

    Returns de-waved beam velocities (not time-averaged).
    """
    nz, nt = b.shape
    bveldw = np.empty((nz, nt))*np.nan
    b = b.T # Each row is a timestamp.
    for k in range(nz-sep):
        ub1 = b[:,k]
        ub2 = b[:,k+sep]
        ...
        # Remove time-means.
        ub1 = ub1 - np.nanmean(ub1)
        ub2 = ub2 - np.nanmean(ub2)
